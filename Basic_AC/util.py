...
class Logger(object):
    def __init__(self, logfile):
        ...
        self.last_write = 0
        self.f.write('step avg_rew ev_before ev_after act_loss crit_loss kl_dist\n')
        self._reset()
        
    def __call__(self, t, act_loss, circ_loss, kl_dist, avg_rew, print_tog, act_lr, worker_id=0, ev_before= -1 , ev_after=-1):
        if print_tog:
            print('Iteration %d' % t)
    ...
    def write(self):
        ...
        for i in range(n):
            ...
            self.f.write('%d %.4f %.4f  %.4f %.4f  %.4f %.4f\n' % (i + self.last_write, avg_rew, ev_before, ev_after,
                                                                        act_loss, circ_loss, kl_dist))
        self._reset()
        self.last_write = + n
