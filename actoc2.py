...
def dense(name, inp, in_dim, out_dim, activation=None, initializer=xavier):
    ...
    b = tf.get_variable(initializer= tf.constant([0.0]*out_dim), name=name+'bias')
...
class Actor(object):
    def __init__(self, num_ob_feat, num_ac, init_lr = 0.001, init_beta = 1, 
...
with tf.Session() as sess:
    ...
    for i in range(ITER):
        ...
        logger(i, act_loss1=act_loss1, act_loss2=act_loss2,  act_loss_full=act_loss_full, circ_loss=cir_loss, 
                avg_rew=avg_rew, print_tog= (i %20) == 0)
        if i % 100 == 50:
            logger.write()
